...
**Beyond the table, ten times a Frontier landscape is a design target** (owner, 2026-09-18), not a fifth class and not work for today: `OpenQuestions.md` Q19 asks whether ten is on the side or on the area before a row is added, because it scales every figure in `TechnicalDesign.md` §4.4 by ten. It is a gameplay question before an engineering one — a heavy device already crosses a Large landscape in 19 minutes, so ten times a Frontier side is a crossing measured in hours, and the crossing-time test this section schedules before M1 is where the useful answer comes from. What the target has already changed is the renderer's structure: ADR-007 records that nothing resident may be O(area), which is why the chunk grid becomes a quadtree rather than keeping a coarse level for every chunk.
