...
| 007 | The view budget: the fog range absolute in world units with a ceiling on the desaturation, superseding 005's fractions on the owner's frame; terrain chunk residency bounded by radius with the coarsest level resident everywhere, so render cost stops scaling with the landscape; the renderer's distances explicitly not line of sight | The owner's frame of 2026-09-18 (written 2026-09-18); the residency it specifies is built by `m3-multiplayer/T11` and measured by `m4-frontier/T1` |
