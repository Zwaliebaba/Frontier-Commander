...
**Every figure above is O(area), and a landscape ten times a Frontier one is a design target** (owner, 2026-09-18; `OpenQuestions.md` Q19 asks whether ten is on the side or on the area, which scales all of this by ten). On the side it is 40,961 samples across: **3.13 GB of heights**, 0.39 GB of cell grid and 0.98 GB of visibility, so 4.5 GB of simulation state before anything is drawn, and 409,600 clusters against the 4,096 §4.5 sizes the cluster graph for. A resident `std::vector` of heights does not reach that, and it is a larger wall than the renderer's, which ADR-007 answers with a quadtree. A landscape is generated from a seed and a tile list, so heights are a pure function of a few hundred bytes and could be produced per region rather than stored per map; whether that is cheap enough for the simulation's random access is `m4-frontier/T1`'s to measure. What does hold at that size: positions, because ADR-002's `std::int32_t` at 1/256 of a world unit spans ten times a Frontier side with 12.8× to spare, and the state hash, because it covers the definition and never the heightfield.
...
**The budget is not what holds as landscapes grow** (ADR-007, 2026-09-18). It bounds triangles, and triangles are not the binding constraint: every chunk of a Frontier landscape at the coarsest stride is 655,360 triangles against a budget of a million, so the budget never engages there. What does not survive is that every chunk is built at all four strides at construction and held for the match — 621 KiB a chunk, so 644 MB resident and some 24 seconds of mesh building on Frontier, neither of which the budget can see. Residency is therefore bounded instead of the view: the coarsest level, 1.6% of a chunk, stays resident for the whole landscape so any chunk can always be drawn, and strides 1, 2 and 4 are built only within a radius of the camera and released beyond it. Resident terrain becomes about 59 MB on every size class, nothing is culled by distance, and the player sees no change. The next wall is draw calls — 1,024 chunks on Frontier before the frustum takes its share — which `m4-frontier/T1` measures. The renderer's distances are **not** line of sight: the Fog pass above reads the commander's visibility as the replica knows it, integer and deterministic under `AGENTS.md` R16, and no render distance may ever feed it.
