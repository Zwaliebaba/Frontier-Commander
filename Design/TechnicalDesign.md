...
**The simulation validates every order** against what the seat owns, can see and can afford, and drops what fails with a reason the client can display: `Sim/OrderValidation.h`'s `RejectReason` is that vocabulary — `NotOwned`, `NotVisible`, `CannotAfford`, `AtCap`, `InvalidTarget`, `InvalidPlacement`, `NotResearched`, `NoCommandPost`, `Malformed` — and a seat carries the orders its own tick refused, in the order they were judged, for Net to report. Validation is one function, so a system arriving later adds its kind's rules there rather than in its own stage, and `Sim/Order.h` carries the operand layout of every one of the twenty kinds. One rewrite rather than a rejection: an `Attack` on a target the seat cannot see becomes an `AttackMove` to the target's last known position from the ghost store (`GameDesign.md` §8), or is refused `NotVisible` when there is no record of it. Validation is inside `Sim`, on the host, and it is the whole of the trust model: a client's order is a request, and a client that lies about what it sees gains nothing, because the host decides visibility (§4.6) and applies orders against its own state.
...
**THE SLACK OF ONE DOES NOT ACTUALLY WORK, measured 2026-09-19.** The sentence above is what the code intends and `Sim/AiSeat.cpp` says so in a comment, and both are wrong: the behaviour that walks a builder to an unfinished site answers *before* the slack is ever consulted, and it answers `true` for as long as there is a site and an idle builder — including when the site is one no builder can be got to. So a commander with an unreachable site never reaches the generator, the factory or the lab at all, and the slack that was written for exactly that case can never be used. It takes a perturbation to see it, because the tree's own seed happens not to produce one; raising the viewer's eye by 16 world units produces one on every seed tried, which is how it was found. `m1-vertical-slice/S13` owns the fix, and its note records the two traps: the guard belongs in one place because `Scout` has the same shape, and component equality is not a reachability test because `ClusterGraph`'s components are local to a cluster.
---

## 8. Content

**Game data is files** (owner, 2026-09-17: R13 withdrawn), **in JSON** (owner, 2026-09-17), under a `GameData\` directory beside the executable, resolved from the executable's own path and never from the working directory. It was `Content\` until 2026-09-18, when the owner separated it from the `Content` project: that library is the loaders, this tree is what they load, and one name for both had put textures and tables inside a source directory. In the repository it is `GameData/` at the root. **Getting it beside the executable is the owner's step** (owner, 2026-09-18), not a build target: nothing in the projects copies it, and a run that does not find it says so and falls back where it can — the terrain to its built-in gradient, the loaders to a refusal. R14 still holds, so the JSON reader is written into `Core`: a strict parser of the standard grammar, about three hundred lines, with the file, line and column in every error. The layout:
