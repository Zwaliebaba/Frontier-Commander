# Frontier Commander — Implementation Plan
...
- **The palette in M0 is a generated gradient**; the Species palettes arrive with ADR-006 in M1 C4, so that no Species-derived file is in the tree before the ADR that records it.
...
| **`FXCompile` and `dxc` on the pinned toolset** | Whether MSBuild's shader step drives dxc for shader model 6 is unknown (`TechnicalDesign.md` §6.2) | M0 T18 finds out first and records the answer in ADR-002; 5.1 through `fxc` is the fallback and the design does not depend on 6 |
